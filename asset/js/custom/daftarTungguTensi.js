...
play_audio = async (data) => {
    const poli = document.getElementById("aliases").value;
    let uri_suara;
    let teks = data.str;
    $('#nomer').html(teks);
    if (loc == 'kkpm.local') {
        if (window.location.protocol == 'http:') {
            // ganti url karo file anyar men ora error pas double panggil
            uri_suara = `http://suara.kkpm.local/getvoice-new.php?id=${new Date().getTime()}&from=${poli}&teks=${teks.toLowerCase()}`;
        } else {
            uri_suara = 'https://bkpm.kentoes.com/auahgelap/getvoice.php?teks=' + teks.toLowerCase();
        }
    } else {
        uri_suara = `http://192.168.100.252/auahgelap/getvoice-new.php?id=${new Date().getTime()}&from=${poli}&teks=${teks.toLocaleLowerCase()}`;
    }

    // kie janen nganggo sing lawas kyne esih bisa
    // cuman tak ganti men q penak wkwkwk 
    const req = await fetch(uri_suara, {
        cache: "no-cache",
    })
...
play_suara = (res) => {
    const audioBell = new Audio(base_uri + 'asset/sound/dingdong.wav');
    const snd = new Audio(res);
